...
export function registerHoverProvider(context: vscode.ExtensionContext) {
  const hover = vscode.languages.registerHoverProvider(['typescript', 'typescriptreact'], {
    async provideHover(document, position, _token) {
      try {
        const inferredType = await getInferredType(document, position)
        if (inferredType) {
          const showTypeOnHover = vscode.workspace.getConfiguration('cssToTyped').get('showTypeOnHover')
          const contents = new vscode.MarkdownString()
          contents.isTrusted = true
          contents.supportHtml = true

          const hoverKey = `${document.uri.toString()}:${position.line}:${position.character}`
          ...
          contents.appendMarkdown(`<a href="command:cssToTyped.copyInferredType">Copy Inferred Type</a> | `)
          contents.appendMarkdown(`<a href="command:cssToTyped.copyConstantObject">Copy Constant Object</a>`)
