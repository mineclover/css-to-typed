...
function findNodeAtPosition(sourceFile: ts.SourceFile, offset: number): ts.Node | undefined {
...
function formatType(type: ts.Type, typeChecker: ts.TypeChecker, depth: number = 0): string {
  // 재귀 깊이 제한
  if (depth > 10) {
    return '...'
  }
  ...
  if (type.isUnion()) {
    return type.types.map((t) => formatType(t, typeChecker, depth + 1)).join(' | ')
  }
  ...
  if (type.isClassOrInterface()) {
    const props = type.getProperties().map((prop) => {
      ...
      return `${prop.name}${isOptional ? '?' : ''}: ${formatType(propType, typeChecker, depth + 1)}`
    })
    return `{ ${props.join('; ')} }`
  }
