...
import { copyTextToClipboard, createFile } from './fileUtils'
import { arrayToStringTyped } from './typeUtils'
import { getDocumentText, getFilePath } from './vscodeUtils'

export function activate(context: vscode.ExtensionContext) {
  const createType = vscode.commands.registerCommand('extension.createType', () => {
  ...
  const clipboardType = vscode.commands.registerCommand('extension.clipboardType', () => {
    const editor = vscode.window.activeTextEditor
    if (!editor) return
